...
class Process:
	def __init__(self, ID=0, arrivalTime=0, burstTime=0):
		self.ID = ID
...
def shortestBurstTime(Processes, currentTime=0) -> int():
...
def SJF(Processes=[]):
	...
	while totalRemainingTime > 0:
		...
		index = shortestBurstTime(Processes, currentTime)
		if index == -1:
			currentTime += 1
			continue
		...
		runTimes[index].append([currentTime - executionTime, executionTime])
